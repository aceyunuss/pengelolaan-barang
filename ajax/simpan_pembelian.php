<?php 
	...
	for($i = 0; $i < count($_POST["hidden_kdobat"]); $i++) {
		...
		$query_pbldtl = "INSERT INTO tbl_pembeliandetail (no_faktur, kd_barang, exp_obatbeli, hrg_beli, jml_beli, sat_beli, subtotal) VALUES ('$no_faktur', '$kd_barang', '$exp_obat', '$hrg_beli', '$jml_obat', '$sat_jual', '$subtotal')";
		mysqli_query($conn, $query_pbldtl) or die ($conn->error);

		$query_stok = "SELECT stk_obat, hrgbeli_obat FROM tbl_databarang WHERE kd_barang = '$kd_barang'";
		...
		$query_exp = "SELECT stok FROM tbl_stokexpbarang WHERE kd_barang = '$kd_barang' AND tgl_exp = '$exp_obat'";
		...
		if($rows_exp > 0) {
			...
			$query_updstokexp = "UPDATE tbl_stokexpbarang SET stok='$stok_baruexp' WHERE kd_barang='$kd_barang' AND tgl_exp = '$exp_obat'";
			mysqli_query($conn, $query_updstokexp) or die ($conn->error);
		} else {
			$query_stokexp = "INSERT INTO tbl_stokexpbarang (kd_barang, tgl_exp, stok) VALUES ('$kd_barang', '$exp_obat', '$jml_obat')";
